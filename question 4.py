    ...
    for i in range(population.shape[0]):
        ...
        for j in range(population.shape[1]):
            if(population[i][j] * prix[j] > 0.2 * budget):
                depassement += population[i][j] * prix[j] - 0.2 * budget
        
        if depassement > 0:
            fitness[i] = min(-depassement,fitness[i]-depassement) #Si la fitness est déjà négative, on y ajoute le dépassement, sinon on utilise juste le dépassement en ignorant la fitness positive
